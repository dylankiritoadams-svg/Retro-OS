...
export const CloudMixer: React.FC<AppProps> = ({ isActive, instanceId, documentIdToOpen }) => {
    ...
    const lineCollision = (p: Particle, obs: {x1:number, y1:number, x2:number, y2:number}) => {
         ...
         if (distSq < p.radius * p.radius) {
             ...
             p.vx -= 2 * dot * normal.x * 0.8;
             p.vy -= 2 * dot * normal.y * 0.8;
         }
    };
    ...
    const animate = useCallback(() => {
        ...
        for (let i = 0; i < particlesRef.current.length; i++) {
            ...
            if (p.x < p.radius || p.x > canvas.width - p.radius) p.vx *= -0.8;
            if (p.y < p.radius || p.y > canvas.height - p.radius) p.vy *= -0.8;
                ...
                <canvas ref={canvasRef} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} className="cursor-crosshair"/>
                 ...
                 <div className="absolute inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
                    <div className="bg-white p-4 border-2 border-black window-shadow text-black">
                        ...
                        <input type="text" value={saveAsName} onChange={e => setSaveAsName(e.target.value)} onKeyPress={e => e.key === 'Enter' && confirmSaveAs()} className="w-full p-1 border-2 border-black bg-white focus:outline-none" autoFocus/>
