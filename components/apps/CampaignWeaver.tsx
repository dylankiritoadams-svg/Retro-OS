    ...
    return (
        <div className="p-2 border-2 border-dashed border-gray-400 mt-4">
            ...
            <div className="flex space-x-2">
                <input type="text" value={prompt} onChange={e => setPrompt(e.target.value)} placeholder={placeholder} className="flex-grow p-1 border-2 border-black" />
...
const WikiEditorPanel: React.FC<{ item: CampaignWikiPage; onUpdate: (data: Partial<CampaignWikiPage>) => void; onLinkClick: (name: string, type: string) => void }> = ({ item, onUpdate, onLinkClick }) => (
    <div className="space-y-4">
        <input type="text" value={item.name} onChange={e => onUpdate({ name: e.target.value })} className="w-full p-1 text-xl font-bold border-2 border-black" placeholder="Article Title" />
        <LinkedTextarea label="Content" value={item.content} onChange={e => onUpdate({ content: e.target.value })} onLinkClick={onLinkClick} />
    </div>
);
...
const NpcEditorPanel: React.FC<{ item: CampaignNpc; onUpdate: (data: Partial<CampaignNpc>) => void; onLinkClick: (name: string, type: string) => void }> = ({ item, onUpdate, onLinkClick }) => (
    <div className="space-y-4">
        <input type="text" value={item.name} onChange={e => onUpdate({ name: e.target.value })} className="w-full p-1 text-xl font-bold border-2 border-black" placeholder="NPC Name" />
        <LinkedTextarea label="Description" value={item.description} onChange={e => onUpdate({ description: e.target.value })} onLinkClick={onLinkClick} />
        <LinkedTextarea label="Secrets" value={item.secrets} onChange={e => onUpdate({ secrets: e.target.value })} onLinkClick={onLinkClick} />
...
const LocationEditorPanel: React.FC<{ item: CampaignLocation; onUpdate: (data: Partial<CampaignLocation>) => void; onLinkClick: (name: string, type: string) => void }> = ({ item, onUpdate, onLinkClick }) => (
    <div className="space-y-4">
        <input type="text" value={item.name} onChange={e => onUpdate({ name: e.target.value })} className="w-full p-1 text-xl font-bold border-2 border-black" placeholder="Location Name" />
        <LinkedTextarea label="Description" value={item.description} onChange={e => onUpdate({ description: e.target.value })} onLinkClick={onLinkClick} />
...
const FactionEditorPanel: React.FC<{ item: CampaignFaction; onUpdate: (data: Partial<CampaignFaction>) => void; onLinkClick: (name: string, type: string) => void }> = ({ item, onUpdate, onLinkClick }) => (
    <div className="space-y-4">
        <input type="text" value={item.name} onChange={e => onUpdate({ name: e.target.value })} className="w-full p-1 text-xl font-bold border-2 border-black" placeholder="Faction Name" />
        <LinkedTextarea label="Goals" value={item.goals} onChange={e => onUpdate({ goals: e.target.value })} onLinkClick={onLinkClick} />
        <LinkedTextarea label="Resources" value={item.resources} onChange={e => onUpdate({ resources: e.target.value })} onLinkClick={onLinkClick} />
...
const ItemEditorPanel: React.FC<{ item: CampaignItem; onUpdate: (data: Partial<CampaignItem>) => void; onLinkClick: (name: string, type: string) => void }> = ({ item, onUpdate, onLinkClick }) => (
    <div className="space-y-4">
        <input type="text" value={item.name} onChange={e => onUpdate({ name: e.target.value })} className="w-full p-1 text-xl font-bold border-2 border-black" placeholder="Item Name" />
        <LinkedTextarea label="Description" value={item.description} onChange={e => onUpdate({ description: e.target.value })} onLinkClick={onLinkClick} />
        <LinkedTextarea label="Properties" value={item.properties} onChange={e => onUpdate({ properties: e.target.value })} onLinkClick={onLinkClick} />
...
const StoryArcEditorPanel: React.FC<{ item: CampaignStoryArc; onUpdate: (data: Partial<CampaignStoryArc>) => void; onLinkClick: (name: string, type: string) => void }> = ({ item, onUpdate, onLinkClick }) => (
     <div className="space-y-4">
        <input type="text" value={item.name} onChange={e => onUpdate({ name: e.target.value })} className="w-full p-1 text-xl font-bold border-2 border-black" placeholder="Story Arc Name" />
        <LinkedTextarea label="Summary" value={item.summary} onChange={e => onUpdate({ summary: e.target.value })} onLinkClick={onLinkClick} />
        <LinkedTextarea label="Key Events" value={item.keyEvents} onChange={e => onUpdate({ keyEvents: e.target.value })} onLinkClick={onLinkClick} />
        <LinkedTextarea label="Linked Lore" value={item.linkedLore} onChange={e => onUpdate({ linkedLore: e.target.value })} onLinkClick={onLinkClick} />
        <LinkedTextarea label="Resolution" value={item.resolution} onChange={e => onUpdate({ resolution: e.target.value })} onLinkClick={onLinkClick} />
...
const SessionEditorPanel: React.FC<{ item: CampaignSession; onUpdate: (data: Partial<CampaignSession>) => void; onLinkClick: (name: string, type: string) => void }> = ({ item, onUpdate, onLinkClick }) => (
     <div className="space-y-4">
        <input type="text" value={item.name} onChange={e => onUpdate({ name: e.target.value })} className="w-full p-1 text-xl font-bold border-2 border-black" placeholder="Session Name" />
        <LinkedTextarea label="Summary" value={item.summary} onChange={e => onUpdate({ summary: e.target.value })} onLinkClick={onLinkClick} />
        <LinkedTextarea label="Planned Events" value={item.plannedEvents} onChange={e => onUpdate({ plannedEvents: e.target.value })} onLinkClick={onLinkClick} />
        <LinkedTextarea label="Linked Lore" value={item.linkedLore} onChange={e => onUpdate({ linkedLore: e.target.value })} onLinkClick={onLinkClick} />
        <LinkedTextarea label="Post-Session Notes" value={item.postSessionNotes} onChange={e => onUpdate({ postSessionNotes: e.target.value })} onLinkClick={onLinkClick} />
...
export const CampaignWeaver: React.FC<{ instanceId: string, documentIdToOpen?: string }> = ({ instanceId, documentIdToOpen }) => {
    ...
    // FIX: Added a double type assertion `as unknown as T[]` to fix a TypeScript error where the compiler couldn't correlate the generic type `T` with the specific property being accessed via `key`. This is safe because the application logic ensures the types always match.
    ...
    const handleAdd = (key: keyof CampaignDocumentContent) => {
        const newItem: CampaignEntity = { id: `${key}-${Date.now()}`, name: '' };
        if (key === 'wikiPages') (newItem as CampaignWikiPage).content = '';
        if (key === 'boards') {
            (newItem as CampaignBoard).boardState = { nodes: [], connections: [], viewport: { x:0, y:0, zoom: 1 }};
        }
