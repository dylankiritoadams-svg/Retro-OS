...
const BoardNode: React.FC<{ node: AnyBoardNode, onDragStart: (e: React.MouseEvent, id: string) => void }> = ({ node, onDragStart }) => {
    ...
    if (node.type === 'note') {
        return <div style={{ ...baseStyle, backgroundColor: node.color }} onMouseDown={e => onDragStart(e, node.id)} className="p-2 whitespace-pre-wrap overflow-hidden">{node.content}</div>
    }
    
    if (node.type === 'arc') {
        return <div style={{ ...baseStyle, backgroundColor: node.color }} onMouseDown={e => onDragStart(e, node.id)} className="p-2 font-bold text-center flex items-center justify-center">{node.entityType}</div>
    }
    
    if (node.type === 'lore') {
        return <div style={{ ...baseStyle, backgroundColor: node.color || '#eee' }} onMouseDown={e => onDragStart(e, node.id)} className="p-2 text-center flex items-center justify-center">{node.entityType}</div>
    }
...
export const CampaignWeaver: React.FC<{ instanceId: string, documentIdToOpen?: string }> = ({ instanceId, documentIdToOpen }) => {
    ...
    // Fix: Using String() wrapper for keys to avoid potential symbol-related runtime errors and improved type safety
    const handleUpdate = <T extends CampaignEntity>(key: keyof CampaignDocumentContent, id: string, data: Partial<T>) => {
        updateCampaign({
            [String(key)]: ((campaign[key] as unknown as T[]).map(item => item.id === id ? { ...item, ...data } : item))
        } as any);
    };
    ...
    const handleAdd = (key: keyof CampaignDocumentContent) => {
        ...
        updateCampaign({ [String(key)]: [...(campaign[key] as any[]), newItem] } as any);
    };

    const handleDelete = (key: keyof CampaignDocumentContent, id: string) => {
        updateCampaign({ [String(key)]: (campaign[key] as any[]).filter(item => item.id !== id) } as any);
    };
