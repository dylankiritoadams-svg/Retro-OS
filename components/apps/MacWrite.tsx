...
export const MacWrite: React.FC<MacWriteProps> = ({ isActive, instanceId, documentIdToOpen }) => {
    ...
    const handleProofread = useCallback(async () => {
        ...
        try {
            const corrected = await proofreadText(originalText);
            // Replace text content while keeping basic formatting if possible, 
            // but innerText replacement is safest for plain corrections in this retro context.
    ...
    useEffect(() => {
        ...
        const eventHandlers: { [key: string]: (data?: any) => void } = {
            ...
            'macwrite:edit:proofread': handleProofread,
    ...
    }, [isActive, instanceId, handleNew, handleOpen, handleSave, handleSaveAs, executeCommand, handleProofread]);

    return (
        <div className="w-full h-full flex flex-col bg-white text-black macwrite-paper relative">
            <div
                ref={editorRef}
                contentEditable
                spellCheck="true"
                ...
                onInput={onInput}
                className="flex-grow p-4 macwrite-editor focus:outline-none overflow-y-auto"
                style={{ fontFamily: FONT_MAP.chicago }}
            />
            {isProcessing && (
                <div className="absolute top-2 right-2 px-2 py-1 bg-black text-white text-[10px] font-bold uppercase animate-pulse">
                    Smart Correcting...
                </div>
            )}
            {isSaveAsModalOpen && (
