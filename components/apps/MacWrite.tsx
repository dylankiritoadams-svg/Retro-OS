...
export const MacWrite: React.FC<MacWriteProps> = ({ isActive, instanceId, documentIdToOpen }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col bg-gray-200 text-black select-none">
            {/* Ruler */}
            <div className="w-full h-6 bg-white border-b-2 border-black flex-shrink-0 px-2 flex items-center text-xs">
                 <span className="font-mono">1"</span>
                 <div className="flex-grow border-b border-gray-400 border-dotted mx-2"></div>
            ...
            <div className="flex-grow bg-white p-4 overflow-y-auto">
                <div
                    key={activeDocument ? activeDocument.id : 'new-doc'}
                    ref={editorRef}
                    contentEditable
                    className="w-full h-full focus:outline-none select-text"
                    onInput={onInput}
                    style={{ fontFamily: FONT_MAP.chicago, fontSize: '12pt', color: 'var(--text-color)' }}
                />
            </div>
            {isSaveAsModalOpen && (
