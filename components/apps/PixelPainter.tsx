...
interface ToolButtonProps {
    toolName: PixelPainterTool;
    activeTool: PixelPainterTool;
    onSelect: (t: PixelPainterTool) => void;
    children: React.ReactNode;
}

const ToolButton: React.FC<ToolButtonProps> = ({ toolName, activeTool, onSelect, children }) => (
    <button 
        onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); onSelect(toolName); }} 
        className={`p-2 border-2 border-black ${activeTool === toolName ? 'bg-black text-white' : 'bg-white'}`}
    >
        {children}
    </button>
);
...
export const PixelPainter: React.FC<AppProps> = ({ isActive, instanceId, documentIdToOpen }) => {
    ...
    useEffect(() => {
        ...
        // Fix: Use an unknown cast to convert a plain string array to PixelPainterTool array to avoid the TS conversion error.
        const eventHandlers: { [key: string]: () => void } = {
            ...
            ...Object.fromEntries((['pencil', 'eraser', 'fill', 'rectangle', 'oval', 'line'] as unknown as PixelPainterTool[]).map(t => [`painter:tools:${t}`, () => setTool(t)])),
        };
    ...
    return (
        <div className="w-full h-full flex flex-row bg-gray-200">
            <div className="flex flex-col items-center p-1 border-r-2 border-black bg-gray-100 space-y-1">
                <ToolButton toolName="pencil" activeTool={tool} onSelect={setTool}>✏️</ToolButton>
                <ToolButton toolName="eraser" activeTool={tool} onSelect={setTool}>🧽</ToolButton>
                <ToolButton toolName="line" activeTool={tool} onSelect={setTool}>╱</ToolButton>
                <ToolButton toolName="fill" activeTool={tool} onSelect={setTool}>💧</ToolButton>
                <ToolButton toolName="rectangle" activeTool={tool} onSelect={setTool}>⬜</ToolButton>
                <ToolButton toolName="oval" activeTool={tool} onSelect={setTool}>⚪</ToolButton>
            </div>
