...
import { PinBoardDayView } from './PinBoardDayView';
...
const PinBoardNodeComponent: React.FC<{
    ...
    switch (node.type) {
        ...
        case 'document':
            ...
            // Fix: cast docAppDef.icon to any to avoid className prop error during cloning
            const docIcon = docAppDef?.icon ? React.cloneElement(docAppDef.icon as React.ReactElement<any>, { className: "h-8 w-8" }) : <DocumentIcon className="h-8 w-8" />;
            content = file ? (
        ...
        case 'app':
            ...
            // Fix: cast appDef.icon to any to avoid className prop error during cloning
            const appIcon = appDef?.icon ? React.cloneElement(appDef.icon as React.ReactElement<any>, { className: "h-10 w-10" }) : <DocumentIcon className="h-10 w-10" />;
            content = appDef ? (
                 <div className="flex flex-col items-center justify-center text-center">
                    {appIcon}
            ...
            <div className="bg-gray-800 p-2 overflow-y-auto">
                {APPS.filter(app => !new Set(['finder', 'settings', 'stickynote-window']).has(app.id)).map(app => (
                    <button key={app.id} onClick={() => onSelect(app.id)} className="w-full text-left p-2 hover:bg-gray-600 flex items-center text-white">
                        {/* Fix: cast app.icon to any to avoid className prop error during cloning */}
                        {React.cloneElement(app.icon as React.ReactElement<any>, { className: "h-6 w-6 mr-2 flex-shrink-0" })}
                        <span>{app.name}</span>
                    </button>
                ))}
            </div>
...
export const PinBoard: React.FC<PinBoardProps> = ({ isOpen, onClose }) => {
    ...
    const [isDayViewVisible, setIsDayViewVisible] = useState(true);
    ...
    const animDuration = animDurations[theme.animationSpeed || 'smooth'];

    return (
        <div 
            className={`absolute inset-0 bg-black bg-opacity-50 z-40 transition-transform ease-[cubic-bezier(0.16,1,0.3,1)] ${isOpen ? 'translate-y-0 pointer-events-auto' : '-translate-y-full pointer-events-none'}`}
            style={{ transitionDuration: animDuration }}
        ...
        >
            ...
            >
                {/* Fixed Widget: Day View - Now at top-left */}
                {isDayViewVisible && (
                    <div className="absolute left-4 top-4 z-50 pointer-events-auto" onMouseDown={e => e.stopPropagation()}>
                        <PinBoardDayView onClose={() => setIsDayViewVisible(false)} />
                    </div>
                )}

                {/* Actions at bottom-right */}
                <div className="absolute bottom-4 right-4 z-10 flex flex-col space-y-2 items-end" onMouseDown={e => e.stopPropagation()}>
                    {!isDayViewVisible && (
                        <button onClick={() => setIsDayViewVisible(true)} className="px-3 py-1 bg-white text-black text-sm shadow-lg border border-black hover:bg-gray-200">Show Day View</button>
                    )}
                    <button onClick={() => addNote({ x: 50, y: 50 })} className="px-3 py-1 bg-yellow-400 text-black text-sm shadow-lg border border-black hover:bg-yellow-300">Add Note</button>
                    <button onClick={() => setIsDocPickerOpen(true)} className="px-3 py-1 bg-green-400 text-black text-sm shadow-lg border border-black hover:bg-green-300">Pin Document...</button>
                    <button onClick={() => setIsAppPickerOpen(true)} className="px-3 py-1 bg-blue-400 text-black text-sm shadow-lg border border-black hover:bg-blue-300">Pin App...</button>
                    <button onClick={onClose} className="px-3 py-1 bg-red-500 text-white text-sm shadow-lg border border-black hover:bg-red-400">Close Board</button>
                </div>
