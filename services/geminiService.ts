...
// --- Proofreading Utility ---
export const proofreadText = async (text: string): Promise<string> => {
    const prompt = `You are a professional proofreader. Correct all spelling, grammar, and punctuation errors in the following text. Preserve the original meaning and tone. Only return the corrected text, no explanations:\n\n${text}`;
...
// --- Planner X Batch Ingestion ---
export const batchIngestEvents = async (
    text: string, 
    currentDate: string, 
    taskHistory: { title: string, duration: number }[]
): Promise<{ events: { title: string, category: string, startTime: string, duration: number, isLocked: boolean }[] }> => {
    
    const historyContext = taskHistory.length > 0 
        ? `Here are some past events and their typical durations to help you infer: ${taskHistory.slice(0, 20).map(t => `${t.title}: ${t.duration}m`).join(', ')}.` 
        : '';

    const prompt = `You are an intelligent scheduling assistant. Parse the following unstructured text into a list of calendar events. 
    
    Current Date: ${currentDate}.
    ${historyContext}

    Rules:
    1. Extract Title, Date/Time, Duration, and Category.
    2. Category must be one of: 'Work', 'Personal', 'Media', 'Appointment'.
    3. If time is missing or ambiguous, default to 9:00 AM on the mentioned day.
    4. If duration is missing, try to infer from history or context. If unknown, default to 30 minutes.
    5. 'isLocked' should be false by default unless the text implies it is immovable (e.g. "strict deadline", "appointment", "flight").
    6. Return startTime as full ISO 8601 string.
    ...
    try {
                    ...
                    properties: {
                        events: {
                            ...
                            items: {
                                ...
                                properties: {
                                    title: { type: Type.STRING },
                                    category: { type: Type.STRING, enum: ['Work', 'Personal', 'Media', 'Appointment'] },
                                    startTime: { type: Type.STRING },
                                    duration: { type: Type.INTEGER },
                                    isLocked: { type: Type.BOOLEAN }
                                },
                                required: ['title', 'category', 'startTime', 'duration', 'isLocked']
                            }
                        }
                    }
                }
        ...
        console.error("Gemini API Error (batchIngestEvents):", e);
        throw new Error(`Failed to ingest events: ${e.message}`);
    }
};
