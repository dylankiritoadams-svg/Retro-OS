...
// Extend the type for internal use to return string
interface ExtendedNotebookContextType extends Omit<NotebookContextType, 'createNode'> {
    createNode: (type: 'page' | 'section', parentId: string, pageType?: NotebookPageType) => string | undefined;
}

const NotebookContext = createContext<ExtendedNotebookContextType | undefined>(undefined);

export const useNotebook = (): ExtendedNotebookContextType => {
...
export const NotebookProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const createNode = useCallback((type: 'page' | 'section', parentId: string, pageType: NotebookPageType = 'mixed') => {
        const parentNode = getNode(parentId);
        if (!parentNode || parentNode.type !== 'section') return undefined;
    ...
    const updatePageContent = useCallback((id: string, content: string) => {
        setState(prevState => {
            ...
            if (!updatedPage.name || updatedPage.name === 'Untitled Page') {
                const firstLine = (content || '').split('\n')[0].trim();
                if (firstLine) updatedPage.name = firstLine.slice(0, 30);
            }
    ...
    const updatePageDrawing = useCallback((id: string, drawingContent: NotebookDrawingData) => {
        setState(prevState => {
            const node = prevState.nodes[id];
            if (!node || node.type !== 'page') return prevState;
            const newNodes = { ...prevState.nodes };
            newNodes[id] = { ...node as NotebookPage, drawingContent };
            return { ...prevState, nodes: newNodes };
        });
    }, []);
    ...
    const value: ExtendedNotebookContextType = {
        nodes: state.nodes,
        rootId: state.rootId,
        getNode,
        getChildren,
