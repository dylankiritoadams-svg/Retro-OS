...
const generateInitialFileSystem = (): Record<string, VFSNode> => {
    const root: VFSFolder = { id: 'root', name: 'Macintosh HD', type: 'folder', parentId: null, childrenIds: ['desktop', 'apps', 'docs', 'network'] };
    ...
    const network: VFSFolder = { id: 'network', name: 'Network', type: 'folder', parentId: 'root', childrenIds: [] };
    ...
    const gameAppIds = new Set(['brick-breaker', 'pong', 'tetris', 'pac-man', 'pixel-pegs', 'peggle-2', 'zurg-cabin', 'old-pc', 'motherload']);
    const utilityAppIds = new Set(['clock', 'planner', 'finder', 'settings', 'stickies', 'task-note', 'up-next', 'bbs-exchange']);
    ...
    APPS.forEach(app => {
        ...
        if (app.id === 'stickynote-window') return;
...
export const FileSystemProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const findNodeByPath = useCallback((path: string): VFSNode | undefined => {
        ...
        for (const part of parts) {
            ...
            currentNode = children.find(child => child.name.toLowerCase() === part.toLowerCase());
    ...
    const deleteNode = useCallback((id: string) => {
        setNodes(prev => {
            ...
            if (nodeToDelete.parentId) {
                const parent = newNodes[nodeToDelete.parentId] as VFSFolder;
                if (parent) {
